#!/usr/bin/env bash
# Phase 2.4 gdb probe launcher v7 — STEP 1 one-shot proof.
# Mirrors nickel-gdb-run-v6.sh staging/env on PORT 1237. No tree changes.
set -euo pipefail
...
for name in ("kobo-touch0.bin", "kobo-button0.bin"):
    ...
    try:
        ...
        pass
    os.mkfifo(p, 0o644)
print("gdb-v7-probe: staged fb backing + empty touch/button FIFOs")
PYEOF

FAKEBIN_LOG="/tmp/opencode/fakebin-gdb-v7.log"
: > "$FAKEBIN_LOG"
...
exec env -i "PATH=$PWD/scripts/fakebin:/usr/bin:/bin" "FAKEBIN_LOG=$FAKEBIN_LOG" \
  ...
  >/tmp/opencode/nickel-gdb-v7.log 2>&1
