#!/usr/bin/env bash
# Phase 2.4 gdb probe launcher — mirrors scripts/run-usermode.sh nickel
# staging (step 5 + identical guest env), but adds qemu-arm -g $PORT.
# Nothing in the tree is modified; all logs go to /tmp/opencode/.
set -euo pipefail
...
for name in ("kobo-touch0.bin", "kobo-button0.bin"):
    ...
    try:
        ...
        pass
    os.mkfifo(p, 0o644)
print("gdb-probe: staged fb backing + empty touch/button FIFOs")
PYEOF

FAKEBIN_LOG="/tmp/opencode/fakebin-gdb-nickel.log"
: > "$FAKEBIN_LOG"
...
exec env -i "PATH=$PWD/scripts/fakebin:/usr/bin:/bin" "FAKEBIN_LOG=$FAKEBIN_LOG" \
  ...
  >/tmp/opencode/nickel-gdb.log 2>&1
